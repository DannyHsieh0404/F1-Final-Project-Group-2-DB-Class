...
def register_user():
    data = request.json
    account = data.get('id')  # Account is email
    ...
    try:
        ...
        """
        # === Core Fix Point ===
        # The second parameter here must use the newly encrypted hashed_password, not the original password
        cursor.execute(query, (account, hashed_password, real_role, "New User", "Not Configured", ""))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
        conn.rollback()
